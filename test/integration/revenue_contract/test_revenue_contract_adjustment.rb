...
class Critic::RevRecContractAdjustment < Critic::RevenueContractValidationHelper
  ...
  # adjust a revenue contract with paginated items
  it 'adjust and order with an item that is paginated' do
      # initial order: 22 items
      # amendment: adjust them all to reduce the period/amount
      ...
      quote_id = create_salesforce_quote(
        ...
        contact_email: "adjust_revenue_contract_with_paginated_items",
      ...
      (0..21).each do |i|
        amendment_data["lineItems"][i]["record"][CPQ_QUOTE_QUANTITY] = 0
      end
      amendment_data["record"][CPQ_QUOTE_SUBSCRIPTION_START_DATE] = format_date_for_salesforce(amendment_start_date)
      amendment_data["record"][CPQ_QUOTE_SUBSCRIPTION_TERM] = amendment_term
      ...
      phase_1 = T.must(subscription_schedule.phases.first)
      assert_equal(22, phase_1.items.count)
      assert_equal(0, phase_1.add_invoice_items.count)

      assert_equal(20, revenue_contract.items.data.count)
      assert(revenue_contract.items.has_more)

      revenue_contract_validate_basics(sf_order, subscription_schedule, revenue_contract, sf_account_id, @defaultSignedDate, version: 2)
      (0..19).each do |i|
        phase_item = T.must(phase_1.items[i])
        contract_item = T.must(revenue_contract.items.data[i])
        revenue_contract_validate_item(phase_item, contract_item, nil, 1, 72000, @defaultTFC)
        revenue_contract_validate_item_period(contract_item, initial_order_start_date, amendment_start_date)
      end

      next_items_page = get_next_revenue_contract_items_page(revenue_contract.id, T.must(revenue_contract.items.data.last).id)
      assert_equal(2, next_items_page.data.count)
      refute(next_items_page.has_more)

      (0..1).each do |i|
        phase_item = T.must(phase_1.items[20 + i])
        contract_item = T.must(revenue_contract.items.data[i])
        revenue_contract_validate_item(phase_item, contract_item, nil, 1, 72000, @defaultTFC)
        revenue_contract_validate_item_period(contract_item, initial_order_start_date, amendment_start_date)
      end
