...
class Critic::ProductTranslation < Critic::VCRTest
  ...
  describe 'mdq products' do
    it 'sync a salesforce order with a mdq licensed product and one-time fee' do
      # setup
      contract_term = 36
      ...
      quote_id = create_salesforce_quote(
        ...
        contact_email: "order_with_mdq_licensed_product_18",
      ...
      # create and add mdq product to quote
      segmented_product_id = create_salesforce_cpq_segmented_product(
        additional_product_fields: {CPQ_QUOTE_BILLING_FREQUENCY => CPQBillingFrequencyOptions::ANNUAL.serialize},
      ...
      # create and add one-time fee to quote
      one_time_fee = 55_00
      one_time_product_id = create_salesforce_cpq_segmented_product(additional_price_dimension_fields: {CPQ_PRICE_DIMENSION_TYPE => CPQPriceDimensionTypeOptions::ONE_TIME.serialize}, price: one_time_fee)
      quote_with_product = add_product_to_cpq_quote(quote_id, sf_product_id: one_time_product_id)
      quote_id = calculate_and_save_cpq_quote(quote_with_product)

      # one yearly-segmented product and one one-time product
      number_of_segments = contract_term / MONTHS_IN_YEAR
      assert_equal(number_of_segments + 1, quote_with_product["lineItems"].count)
      ...
      subscription_schedule.phases.each_with_index do |phase, index|
        ...
        if index == 0
          # the first segment should have a one-time fee
      ...
      # sanity check the sub schedule phase durations
      ...
      # sanity check the total amount
      assert_equal(sf_order["TotalAmount"], ((TEST_DEFAULT_PRICE * number_of_segments) + one_time_fee) / 100)
    end
    ...
    it 'syncing salesforce order with one-time segmented product creates invoice' do
      # setup
      contract_term = 24
      initial_order_start_date = now_time
      initial_order_end_date = initial_order_start_date + contract_term.months
      ...
      quote_id = create_salesforce_quote(
        sf_account_id: sf_account_id,
        contact_email: "order_with_one_time_mdq_licensed_product_3",
        additional_quote_fields: {
          CPQ_QUOTE_SUBSCRIPTION_START_DATE => format_date_for_salesforce(initial_order_start_date),
          CPQ_QUOTE_SUBSCRIPTION_TERM => contract_term,
        }
      ...
      # create and add mdq product to quote
      segmented_product_id = create_salesforce_cpq_segmented_product(
        additional_product_fields: {CPQ_QUOTE_BILLING_FREQUENCY => CPQBillingFrequencyOptions::MONTHLY.serialize},
        additional_price_dimension_fields: {CPQ_PRICE_DIMENSION_TYPE => CPQPriceDimensionTypeOptions::ONE_TIME.serialize})
      quote_with_product = add_product_to_cpq_quote(quote_id, sf_product_id: segmented_product_id)
      quote_id = calculate_and_save_cpq_quote(quote_with_product)

      # there shouldn't be any segments since this is a one-time product
      assert_equal(1, quote_with_product["lineItems"].count)

      # create and translate order
      sf_order = create_order_from_cpq_quote(quote_id)
      sf_order_items = sf_get_related(sf_order, SF_ORDER_ITEM)

      # a One-Time segmented product is still an MDQ product, so it should have a segment index
      assert_equal(1, sf_order_items.count)
      assert_equal(0, sf_order_items.first[CPQ_ORDER_ITEM_SEGMENT_INDEX])
      assert_not_empty(sf_order_items.first[CPQ_ORDER_ITEM_SEGMENT_KEY])
      StripeForce::Translate.perform_inline(@user, sf_order.Id)
      sf_order.refresh
      ...
      assert_equal(sf_order["TotalAmount"], invoice.total / 100)
    ...
    it 'sync salesforce order with one-time and quartely segmented product' do
      # setup
      contract_term = 24
      initial_order_start_date = now_time
      initial_order_end_date = initial_order_start_date + contract_term.months
      ...
      quote_id = create_salesforce_quote(
        sf_account_id: sf_account_id,
        contact_email: "order_with_one_time_mdq_licensed_product_2",
        additional_quote_fields: {
          CPQ_QUOTE_SUBSCRIPTION_START_DATE => format_date_for_salesforce(initial_order_start_date),
          CPQ_QUOTE_SUBSCRIPTION_TERM => contract_term,
        }
      ...
      first_phase = T.must(subscription_schedule.phases.first)
      assert_equal(first_phase.start_date.to_i, initial_order_start_date.to_i)
      assert_equal(first_phase.end_date.to_i, initial_order_end_date.to_i)
    end
  end
end
