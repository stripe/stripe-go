...
class StripeForce::Translate
  ...
  def create_stripe_transaction_from_sf_order(sf_order)
    ...
    if has_evergreen_products
      ...
      validate_evergreen_order_items(sf_order_items)
      return create_stripe_subscription_from_initial_evergreen_sf_order(sf_order, subscription_items, stripe_customer)
    end
    ...
    # note: this field does not exist on the Stripe API so we delete it
    # but it is a required mapping field
    Integrations::Utilities::StripeUtil.delete_field_from_stripe_object(subscription_schedule, :iterations)
  ...
  def generate_phases_for_initial_order(sf_order:, invoice_items:, subscription_items:, subscription_schedule:, stripe_customer:)
    ...
    if is_initial_order_backend_prorated
      ...
      # TODO make sure the initial state on the order amendments includes the second phase
  ...
  def apply_amendment_order_mappings(mapper, subscription_schedule, sf_order_amendment)
    ...
    if @user.feature_enabled?(StripeForce::Constants::FeatureFlags::INVOICE_RENDERING_TEMPLATE)
      ...
      if invoice_rendering_template.present?
