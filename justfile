...
# ⭐ run format, and tests to prepare for CI. Doesn't lint, since that takes too long locally
prepare: format test
...
print-version-info:
    #!/usr/bin/env bash
    set -euo pipefail
    echo "pinned-api-version: $(rg -N --color never -m1 -o '[0-9]{4}-[0-9]{2}-[0-9]{2}[.\w-]*' api_version.go)"
    echo "minimum-runtime-version: $(rg -N --color never -o '^go (.+)' --replace '$1' go.mod)"
