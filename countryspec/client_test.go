...
func TestCountrySpecGet(t *testing.T) {
	...
	if len(target.VerificationFields["individual"]["minimum"]) == 0 {
		t.Errorf("Empty list of minimum verification fields for an individual in the US: %v", target.VerificationFields["individual"]["minimum"])
	}
}
