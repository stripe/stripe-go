...
func TestBackendDetectsContextLogger(t *testing.T) {
	...
	config := &BackendConfig{
		...
		LeveledLogger:     contextLogger, // Pass context logger directly
	...
	// Verify context logger was detected
	assert.NotNil(t, backend.contextLogger, "Backend should detect ContextLeveledLoggerInterface")
	assert.Equal(t, contextLogger, backend.contextLogger, "Backend should store the context logger")
}
...
func TestBackendWorksWithRegularLogger(t *testing.T) {
	...
	// Verify context logger is nil (not detected)
	assert.Nil(t, backend.contextLogger, "Backend should not detect regular LeveledLoggerInterface as ContextLeveledLoggerInterface")
...
// Test nil context handling in helper methods
func TestHelperMethodsWithNilContext(t *testing.T) {
	var stdout, stderr bytes.Buffer
	contextLogger := &testContextLogger{
		LeveledLogger: &LeveledLogger{
			stdoutOverride: &stdout,
	...
	assert.Nil(t, contextLogger.lastContext, "Nil context should be passed as-is")
}

// Test logErrorWithContext helper
func TestLogErrorWithContext(t *testing.T) {
	...
	backend.logErrorWithContext(ctx, 400, stripeErr)
	...
	backend.logErrorWithContext(ctx, 402, stripeErr)
